...
class TestAdvisorInitialization:
    ...
    def test_tools(self):
        """Test advisor has exactly the ten internal tools."""
        advisor = Advisor()
        ...
        assert "audit_feasibility" in tool_names
...
class TestScopedAdvisor:
    ...
    def test_scoped_toolset(self):
        ...
        explore + deepen + audit_feasibility) — only ingest is excluded (bulk
        extraction stays unscoped)."""
        from dialectical_framework.graph.scope_context import scope
