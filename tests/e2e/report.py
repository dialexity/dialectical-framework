...
def build_provenance() -> dict:
    """Which build produced a run: git HEAD, dirty flag, and prompt-file HEAD.

    Recorded because the archive's most-repeated caveat — "every run in
    `results/` was generated by a slightly different build" (`across_runs.py`) —
    was until now unverifiable from the records themselves. Three strong-tier
    A2-vs-A1.7 sets were pooled as one evidence base while
    `advisor/system_prompts.py` took 12 commits between the earliest of them and
    the next run; the only way to notice was `ls -lT` against `git log`, by hand,
    from memory. A pooled estimate that cannot say which prompt it measured is
    not an estimate of anything shippable.

    `prompt_sha` is tracked separately from `git_sha` on purpose: a commit that
    touches only the bench leaves the measured artefact unchanged, and runs on
    either side of it CAN be pooled. `dirty` covers the uncommitted-edit case,
    where the sha names a build that never existed.

    Best-effort by design. A missing git, a tarball checkout, or a subprocess
    failure yields `{"error": ...}` rather than an exception: provenance is
    metadata about hours of paid model time, and losing the run to a bookkeeping
    failure would be the worse outcome.
    """
    import subprocess

    prompt_file = "src/dialectical_framework/agents/advisor/system_prompts.py"
    ...
    try:
        return {
            ...
            "prompt_sha": _git("log", "-1", "--format=%H", "--", prompt_file),
        }
    except Exception as exc:  # noqa: BLE001 — never lose a paid run to bookkeeping
...
def render_report(
    ...
    if thin:
        ...
        add("   DIALEXITY_E2E_REPLICATES before reading any row as a result.")
        add("")
    for (arm_a, arm_b), d in sorted(deltas.items(), key=lambda kv: (kv[0][0].value, kv[0][1].value)):
        ...
        if total_rows:
            ...
            if not resolved_rows:
                ...
                add("      means — raise DIALEXITY_E2E_REPLICATES instead.")
            ...
            if widest and widest[0] > 0:
                ...
                if sd:
                    ...
                    add(
                        "   DIALEXITY_E2E_REPLICATES accordingly BEFORE the run,"
                    )
    ...
    add("   AS OF 2026-08-18 NO CONTROL HAS BEEN READ: poorfit_ssl_expiry and")
    add("   premature_relocation have cells only under smoke* stems, which")
    add("   every pooled read excludes. Item 5 is an instruction with nothing")
    add("   to apply it to. Do not read a run that omits them as having")
    add("   passed a control it never took.")
    return "\n".join(lines)
