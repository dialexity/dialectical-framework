"""Pull the judge's own rationale for the cells an arm LOST, de-randomised.
    ...
    poetry run python tests/e2e/judge_notes.py            # the 5 worst dimensions
    poetry run python tests/e2e/judge_notes.py warmth     # one, named
    poetry run python tests/e2e/judge_notes.py --all-cells      # won cells too
    poetry run python tests/e2e/judge_notes.py --dump /tmp/out  # file per dimension
...
from e2e.across_runs import _stems  # noqa: E402
from e2e.models import Comparison  # noqa: E402
