...
class E2ERun:
    ...
    def __init__(self, container, config: E2EConfig) -> None:
        self._container = container
        self._config = config
        self._driver = E2EDriver(container, simulator_model=config.simulator_model)
    ...
    async def judge_pairs(
        ...
        *,
        ...
        say = progress or (lambda msg: logger.info("%s", msg))
        judge = E2EJudge(self._container, self._config.judge_model)
    ...
    def save(self, directory: Path, *, stem: str = "e2e") -> tuple[Path, Path]:
...
def score_machine_over(
    ...
    Module-level rather than only a `E2ERun` method so it can run over a loaded
    archive without a `E2EConfig` — re-scoring saved transcripts needs no models,
