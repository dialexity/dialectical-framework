...
every future bench row silently mis-flags.
    ...
    poetry run pytest tests/e2e/probe_graph_summary_on_woven_graph.py --real-llm -s
"""
...
async def test_woven_count_tracks_the_seam(di_container) -> None:
    ...
    weak = E2EConfig.from_env(tiers=["weak"]).tiers["weak"]
    case = Case()
    case.commit()
