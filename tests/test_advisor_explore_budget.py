...
settings.advisor_deep_wheels caps deep generation (delivered by task #2),
...
class TestExploreBudget:
    ...
    async def test_deep_wheels_budget_reaches_pipeline(
        self, di_container, stubs
    ):
        from dialectical_framework.agents.advisor.tools.explore import \
            run_exploration

        with _settings(di_container, advisor_deep_wheels=2):
            await run_exploration(["pp1"], intent="", nexus_hash="deadbee")

        assert stubs["pipeline"] == [2]
