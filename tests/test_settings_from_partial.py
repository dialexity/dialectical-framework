...
def env_configured(monkeypatch):
    ...
    monkeypatch.setenv("DIALEXITY_ADVISOR_CONTEXT_MAX_WHEELS", "5")
...
class TestFromPartial:
    def test_none_returns_env(self, env_configured):
        ...
        assert s.advisor_context_max_wheels == 5
    ...
    def test_explicitly_set_fields_override_env(self, env_configured):
        partial = Settings(
            ai_model="openai/other-model", advisor_context_max_wheels=10
        )
        s = Settings.from_partial(partial)
        assert s.advisor_context_max_wheels == 10
    ...
    def test_explicit_default_value_still_overrides(self, env_configured):
        ...
        partial = Settings(ai_model="openai/other-model", advisor_context_max_wheels=3)
        s = Settings.from_partial(partial)
        assert s.advisor_context_max_wheels == 3
