...
Env vars (all optional; defaults below are the value the code uses):
    ...
    DIALEXITY_BENCH_TIER_WEAK   weaker tier under test
    DIALEXITY_BENCH_TIER_STRONG stronger tier under test
    DIALEXITY_BENCH_SIMULATOR   model that plays the person (fixed across arms)
    DIALEXITY_BENCH_JUDGE       model that scores transcripts
...
class BenchConfig(BaseModel):
    ...
    @classmethod
    def from_env(cls, *, tiers: Optional[list[str]] = None) -> "BenchConfig":
        ...
        """
        available = {
            "weak": os.getenv("DIALEXITY_BENCH_TIER_WEAK", DEFAULT_TIER_WEAK),
            "strong": os.getenv("DIALEXITY_BENCH_TIER_STRONG", DEFAULT_TIER_STRONG),
        }
        if tiers:
        ...
        return cls(
            tiers=selected,
            simulator_model=os.getenv("DIALEXITY_BENCH_SIMULATOR", DEFAULT_SIMULATOR),
            judge_model=os.getenv("DIALEXITY_BENCH_JUDGE", DEFAULT_JUDGE),
        )
