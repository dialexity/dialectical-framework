...
Separate from `test_bench.py` because that file is deliberately DB-free and
...
`test_bench.py::TestStanceScore` / `TestMemoryScore`, where the inputs can be
varied.
...
async def test_stance_lane_attaches_a_verdict_per_rung(di_container):
    ...
    run = BenchRun(di_container, BenchConfig.from_env(tiers=["weak"]))
