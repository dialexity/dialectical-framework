...
class CausalityEstimatorDesirable(CausalityEstimatorBalanced):
    ...
    def prompt_assess_multiple_sequences(
        ...
        return [llm.messages.user(
            f"Which of the following circular causality sequences is the most desirable, "
            f"i.e. would produce optimal outcomes and maximum results "
            f"(given that the final step cycles back to the first step):\n"
            f"{sequences_text}\n\n"
            f"<instructions>\n"
            f"For each sequence:\n"
            f"1) Estimate the numeric probability (0 to 1) regarding how beneficial/optimal this sequence would be if implemented\n"
    ...
    def prompt_assess_single_sequence(self, *, sequence: str) -> list:
        return [llm.messages.user(
            f"Assess the following circular causality sequence considering desirability, "
            f"i.e. producing optimal outcomes and maximum results "
            f"(given that the final step cycles back to the first step):\n"
            f"{sequence}\n\n"
            f"<instructions>\n"
            f"1) Estimate the numeric probability (0 to 1) regarding how beneficial/optimal this sequence would be if implemented\n"
