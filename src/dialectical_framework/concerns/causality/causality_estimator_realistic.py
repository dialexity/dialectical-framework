...
class CausalityEstimatorRealistic(CausalityEstimatorBalanced):
    ...
    def prompt_assess_multiple_sequences(
        ...
        return [llm.messages.user(
            f"Which of the following circular causality sequences is the most realistic, "
            f"i.e. what typically happens in natural systems "
            f"(given that the final step cycles back to the first step):\n"
            f"{sequences_text}\n\n"
            f"<instructions>\n"
            f"For each sequence:\n"
            f"1) Estimate the numeric probability (0 to 1) regarding its realistic existence in natural/existing systems\n"
    ...
    def prompt_assess_single_sequence(self, *, sequence: str) -> list:
        return [llm.messages.user(
            f"Assess the following circular causality sequence for realism, "
            f"i.e. what typically happens in natural systems "
            f"(given that the final step cycles back to the first step):\n"
            f"{sequence}\n\n"
            f"<instructions>\n"
