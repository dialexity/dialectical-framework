...
class PerspectiveRepository:
    ...
    def safe_delete(
        ...
        OPTIONAL MATCH (pp)<-[:T_PLUS|T_MINUS|A_PLUS|A_MINUS]-(aspect_component)
        ...
        OPTIONAL MATCH (aspect_component)<-[:EXPLAINS]-(comp_rationale)
        ...
        WITH pp, polarity,
             collect(DISTINCT aspect_component) + collect(DISTINCT polarity_comp) AS components,
    ...
    def is_isolated(
        ...
        WITH pp,
             ...
             collect(DISTINCT aspect_component) + collect(DISTINCT polarity_comp) + collect(DISTINCT synth_comp) AS all_components

        UNWIND CASE WHEN size(all_components) > 0 THEN all_components ELSE [null] END AS comp

        // Check if component is used in other Perspectives directly (aspect positions)
