    ...
    async def redefine(
        ...
        for dialectical_component in [base, other]:
            if changed.get(dialectical_component):
                setattr(new_wu, dialectical_component, DialecticalComponent(
                    alias=new_wu.__pydantic_fields__.get(dialectical_component).alias,
        ...
        if changed.get(base) or changed.get(other):
            check1 = check(is_valid_opposition, getattr(new_wu, base).statement, getattr(new_wu, other).statement)
            ...
            if not check1.valid:
                getattr(new_wu, base).statement = f"ERROR: {getattr(new_wu, base).statement}"
                getattr(new_wu, other).statement = f"ERROR: {getattr(new_wu, other).statement}"
        ...
        for side in ['t', 'a']:
            ...
            if (changed.get(base) or changed.get(base_minus)) or (changed.get(other) or changed.get(other_plus)):
                if changed.get(base_minus) or changed.get(base):
                    check2 = check(is_negative_side, getattr(new_wu, base_minus).statement, getattr(new_wu, base).statement)
                        ...
                        if changed.get(base) and not changed.get(base_minus):
                            ...
                            if hasattr(new_wu, other_minus):
                                if getattr(new_wu, other_minus):
                                    not_like_other_minus = getattr(new_wu, other_minus).statement
                            bm = await base_negative_side_fn(getattr(new_wu, base).statement, not_like_other_minus)
                            assert isinstance(bm, DialecticalComponent)
                            bm.explanation = f"REGENERATED. {bm.explanation}"
                    ...
                    if not check2.valid:
                        getattr(new_wu, base_minus).statement = f"ERROR: {getattr(new_wu, base_minus).statement}"
                ...
                if changed.get(other_plus) or changed.get(other):
                    check3 = check(is_positive_side, getattr(new_wu, other_plus).statement, getattr(new_wu, other).statement)

                    if not check3.valid:
                        if changed.get(other) and not changed.get(other_plus):
                            op = await other_positive_side_fn(getattr(new_wu, other).statement, getattr(new_wu, base_minus).statement)
                    ...
                    if not check3.valid:
                        getattr(new_wu, other_plus).statement = f"ERROR: {getattr(new_wu, other_plus).statement}"
                ...
                if not additional_diagonal_check_skip:
                    check4 = check(is_strict_opposition, getattr(new_wu, base_minus).statement, getattr(new_wu, other_plus).statement)
                    if not check4.valid:
                    ...
                    if not check4.valid:
                        ...
                        raise AssertionError(f"{alias_base_minus}, {alias_other_plus}", warnings, new_wu)
