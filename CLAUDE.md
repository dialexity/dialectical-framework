...
- **Advisor** = pure-conversation agent where framework runs silently. Composes both pipelines via internal tools: `ingest`, `anchor`, `explore`, `deepen`, `sync` (plus shared `inspect_node`, `read_digest`, `discard`). No framework terminology exposed to user. App preamble defines persona; system prompt is domain-neutral dialectical engine (a FUNCTION `system_prompt(tool_names, scoped_nexus_hash)` — tool docs render only for wired tools). Apps add domain-resource tools via `app_tools=` — uniform across all three agents (`agents/toolsets.py::merge_app_tools`): appended `@llm.tool` functions; system prompts skip unknown names — document app tools in the app preamble; shadowing a built-in raises. One `app_tools` list per app, passed to EVERY head: the Explorer↔Advisor toggle shares literal history (missing tool = capability breaks mid-conversation); the Analyst thread owes the same resources by parity (Analyst+Explorer+counsel toggle = one Navigator app).
