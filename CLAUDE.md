...
- **Advisor** = pure-conversation agent where framework runs silently. Composes both pipelines via internal tools: `ingest`, `anchor`, `explore`, `deepen`, `sync` (plus shared `inspect_node`, `read_digest`, `discard`). No framework terminology exposed to user. App preamble defines persona; system prompt is domain-neutral dialectical engine (a FUNCTION `system_prompt(tool_names, scoped_nexus_hash)` — tool docs render only for wired tools). Apps add domain-resource tools via `extra_tools=` — uniform across all three agents (`agents/toolsets.py::merge_extra_tools`): appended `@llm.tool` functions; system prompts skip unknown names — document app tools in the app preamble; shadowing a built-in raises. In an Explorer↔Advisor toggle pass the SAME `extra_tools` to both heads (capability continuity).
