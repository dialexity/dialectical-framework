...
**Everything in this section lives inside `use_brain`, and the tool loop's `resume()` calls do not go through it.** Both continuation sites (`conversation_facilitator.py:258` and `:374`) are Mirascope's own requests, so a tool-round continuation gets no concurrency slot, no rate-limit/ParseError retry, no generation span, and no census record. The turn's FIRST call is covered; rounds 2..N are not.
