...
**Instrumented stages:** `anchor`, `ingest`, `decision`, `synthesis`, `transformation`, `feasibility`. The keyed tool-level scopes (`anchor`, `ingest`) install at the TOOL and delegate to a split-out body, because the expensive work is inside gathers the tool creates — `ingest`'s are `SourceDigest`'s parts and `SurfaceTheses._sweep_windows`' windows, and it is what makes `AnalysisPipeline`'s pre-existing (previously silent) pair fire. Three rules the `ingest` work added, all pinned by `tests/test_ingest_progress.py`: **declare each step at its own site**, never up front — the closing event reports what COMPLETED and is not rounded up, so a step declared for a conditional phase that never runs looks exactly like one that failed; **report inside the semaphore**, so an event means "being read now" rather than "queued" (at a cap of 3 over 33 windows, queueing is most of the wait); and **indices only, never source content** — the unit of work on this path is a slice of the person's own file, so `f"Reading section {i} of {n}"`, never the window. `record_decision` (stage `decision`, keyed, tool-level) is the counter-example that fixes the rules' limit: it declares exactly ONE step, on the coherence check (~5.6s, the only stretch that produces nothing until it returns), because everything else the concern does is validation that refuses in-band and graph writes that already reach the host as effects on the `sid` channel — a second progress step for the same commit would tell the person their decision was saved twice. Its in-band refusals close at **0/0**, which is why `test_ingest_progress.py`'s `assert steps` vacuity guard does not transfer to that branch (`tests/test_decision_progress.py`). **Two tools still have no scope of their own, and the second is a live host defect rather than just a gap.** `explore` has none, so its wheel-building phase is unnarrated. `deepen` has none either, and because it calls `ExploreTransformations` then `GenerateSynthesis` in sequence, each of which installs its OWN scope, one `deepen` call publishes **two `final` events** — a host clears its indicator when the transformation phase ends and then gets a second indicator for synthesis. **The `anchor`/`ingest` shape does NOT transfer here, and it is worth knowing why before anyone reaches for it.** Those two work because nothing beneath them installs a scope — the inner sites only call `report_progress`/`expect_progress`, which is exactly what makes `AnalysisPipeline`'s previously silent pair fire. Wrapping `deepen` the same way would give it THREE `final` events, not one: `progress_scope` installs unconditionally and its `finally` always publishes, so a nested scope closes on its own way out and the outer one closes again after it. Both skills are also entry points in their own right (`explore` calls them, and the Explorer agent reaches them directly), so they cannot simply drop their scopes either. Closing this needs a seam decision first — a scope that defers to an installed one, or a `final` that only the outermost publishes — so treat it as a design question about `utils/progress.py`, not a site fix.
