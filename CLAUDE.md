...
**`ingest`'s instrumentation is VERIFIED on the real path, and the live runs named what is still missing** (`probe_ingest_progress.py`, three runs). At 120 KB the sweep delivers what only a provider can show — a denominator known in advance and reported twice ("Reading part 3 of 4", "Reading section 3 of 4 for tensions") — closing at 23/23 with one key, exactly one terminal `final` event, no phantom steps and no content leaks, and the worst silent stretch cut from **52.3s graph-only to 14.4s**. Two numbers to carry rather than the wall clock: **total dead air is unchanged at 91%** (the channel shortens the worst gap; it does not produce continuous motion), and the denominator's **worst backwards jump is 21%**, which is the live size of the caching caveat above. **The three remaining holes are all the same shape — one label announced before a gathered fan-out** — and the fix differs by case: report per item where the items are staggered, but where they start together (`expand_polarities`' five ~11s tetrads, all announced at 78.8s) only per-CALL subdivision helps, as `TransformationGeneration` has. The one worth taking first is the smallest: the single-window `_extraction_loop` bundles extraction, the step-2 gate and classification under one label — 12.2s of a 45.5s wall — where `_extraction_sweep` already announces the classify phase separately, and single-window is the common chat case.
