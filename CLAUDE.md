...
**Parallelization points:** `ExplorationPipeline` runs wheels concurrently. `ExploreTransformations` parallelizes edge pairs, Phase 1 edges, Phase 2 candidates, audits (when enabled). `AnalysisPipeline` parallelizes `expand_polarities`/`find_polarities`. Graph writes stay sequential after gather.
