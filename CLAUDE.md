...
- **Product model — four CATEGORIES of app, not four products** (the framework cannot predict its applications; owner's framing, 2026-09-21; `docs/agents.md` "Choosing what to build"): **Navigator** = Analyst + Explorer (+ the advisory register), a system scientist builds and navigates the wheel at once, in the open. **Advisor on a nexus** = `Advisor(nexus_hash=, messages=)`, an analyst or mediator exploring ONE constellation with assisted reasoning — the surface built for the framework's own claim; benched as `A2n`: on Haiku equal to the Consultant and below the dump (`nexus-pinned`); on Sonnet 5 above the dump but a resolved loss to the Consultant (`ladder-sonnet`), with no build tool elected inside the pin on either model and the caveat that a Sonnet build makes several nexuses and the bench pins to one. **On Sonnet 5 every live head sits above the static dump of its own graph and the Consultant does so resolved (+0.50 [+0.08, +0.92])** — "live loses to static" was the weak model's verdict. **Advisor from scratch** = `Advisor(app=)`, a mediator's client resolving an issue with a dialectically thinking LLM; builds silently and **collapses into the previous category** once it has a nexus to dive into (`Advisor(nexus_hash=, app=, persona=True)` keeps the persona and hides the machinery over the pin); the benched arm measures the on-ramp only. **Consultant** = `Advisor(mode=CONSULTANT)`, a person talking to a graph something else built — typically the headless builder; the best counsel measured on both models and never worse than the builder (`AnalysisPipeline` → `CreateNexus` → `ExplorationPipeline`, or `run_exploration_detailed`; documented under that heading). `VIEW` is an access level, not a category.
