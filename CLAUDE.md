...
- **Product model — four products** (owner's framing, 2026-09-21; `docs/agents.md` "Choosing what to build"): **Navigator** = Analyst + Explorer, a system scientist builds the graph in the open (chat is the UI, heads switch by resuming with the same `messages`). **Advisor on a nexus** = `Advisor(nexus_hash=, messages=)`, the mediator exploring ONE exploration with assisted reasoning, enriching inside the pin — the surface built for the framework's own claim and the one with ZERO judged cells (every archived A2 cell is unscoped). **Advisor from scratch** = `Advisor(app=)`, an ordinary person with a context; builds silently from nothing; the benched arm — first session loses to a static dump of its own graph, wins at the return. **Consultant** = `Advisor(mode=CONSULTANT)`, anyone talking to an existing graph; never builds, records decisions; fastest and best in-session. `VIEW` is an access level, not a product.
