...
| `FeasibilityEstimation` | Practical achievability (Ac+/Re+ transitions only). Written by three paths: eagerly on every new Transformation when `settings.audit_transformations` is on (default off), automatically on the pathway a recorded decision is grounded on when `settings.automatic_feasibility_audit` is on (default OFF since 2026-09-15, i.e. manual — the tool stays wired either way, so what the flag chooses is who initiates and never whether the band is reachable), or on demand for named pathways via the `audit_feasibility` tool. Its `provider` Rationale carries the factors and success conditions behind the number — the tool renders both |
