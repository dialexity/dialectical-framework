...
| **R2 Circular causality** (Ac+ = this edge's T-→A+, Re+ = the OPPOSITE edge's T-→A+) | `transformation_generation` SYSTEM_PROMPT; `positive_ac_re_apex_derivation`; `action_extraction`; `synthesis_generation`; comments in `ac_re_taxonomy.py` | **Duplicated prose across 4+ prompts, no single owner.** Directionality is theory-critical. **`Re+ = A-→T+` is the 1-Polarity collapse, not the general rule** — it holds only when the edge joins the two sides of one Polarity (`source.opposite == target`), which is why `transformation_generation`'s worked example labels itself "1-PP". For ≥2 Polarities, Re+ runs on `source.opposite → target.opposite`, which is the antipodal edge's Ac+ arrow. `transformation_generation` SYSTEM_PROMPT states the general rule correctly (`opp_source.neg → opp_target.pos`) and generates from `opposite_edge_context`, so generation is sound; `positive_ac_re_apex_derivation` was fixed 2026-09-11. Swept 2026-09-11 (`transformation.py`, `polarity_relationship.py`, `ac_re_taxonomy.py`, `get_schema.py`, `explorer/system_prompts.py`, `docs/graph.md`, plus the Rule 5.1 Notes in `docs/theory/generative-rules.md` — the paper QUOTE there is left verbatim, only our commentary annotated). Locked by `TestReSideLivesOnTheOppositeEdge`, which asserts the builder is the ground truth and that the two model-facing sites no longer teach the collapse. Left deliberately: `transformation_generation`'s worked example (declares itself "1-PP") and citations of Rule 5.1's own wording in `rendering.py` / test docstrings. `docs/graph.md`'s edge diagram was ALSO structurally wrong — it drew a 2-PP wheel as two disjoint 2-cycles (T1↔A2, T2↔A1) instead of one 4-cycle; fixed in the same pass. |
...
| **R6 Control statements** ("T+ without A+ yields T-") | `concerns/control_statements_check.py` (aspect level) AND `transformation_generation` (transition level, `Ac+ without Re+`; also carries the R6 backfire corollary "Never propose direct reinforcement of a '+' aspect", locked by `TestBackfireConstraint`, and the forcefulness→polarity-flip constraint "Forcefulness reverses polarity" — sibling in `action_extraction.py` Requirements #5; both locked by `TestForcefulnessPolarityFlip`) AND `ASPECT_DEFINITIONS` (neutral-T variant: "-" aspects defined as what T/A degenerate into when the opposition's "+" is absent, + truth criterion; locked by `test_aspect_definitions_carry_neutral_degeneration`) | **Duplicated form**, independent wording across check/prompt sites; neutral-T variant single-sourced in the constant. The forcefulness rule is stated 2× (transformation_generation + action_extraction) with independent wording — drift-watch. **The duplication DID drift, and in the worst possible way: until 2026-09-11 the transition-level statement had the SUBJECT SWAPPED** ("Ac+ without Re+ yields Re-"), so the Ac-/Re- prompt bodies each described the other position's mechanism. Rule 5.2 makes the transition tetrad "obedient to rules 3.1–3.3", so it inherits X+ without Y+ → X- and the subject is never the complement. The same prompt stated the rule CORRECTLY two paragraphs later ("flips T+ into T-", "becomes its '-' counterpart"), which is why reading any single site was not enough to catch it, and why the review checklist that was supposed to catch it carried the mirrored version too. Check the subject, not the presence of a "without" sentence. |
...
4. **ApexDerivation apex text → `TransformationGeneration._score_hs`.** Apex-prompt wording sets the HS
   ...
   says so at the Phase-1 pairing comment). Until 2026-09-11 `positive_ac_re_apex_derivation` built one
   context and read both apexes off it, so Re+ HS on any wheel with ≥2 Polarities scored a path against a
   reference through different nodes. It survived because each module was self-consistent — only the pair
   disagreed, so no single-file reading could see it. Cross-module lock:
   `TestApexFramesMatchGenerationFrames`. The two apexes are NOT interchangeable with the opposite edge's
   Ac+ apex despite sharing the arrow: apex zones differ (Re+ ~0.2 proactiveness, Ac+ ~0.6), so a
   reflection reference and an action reference over the same node pair are different references.
